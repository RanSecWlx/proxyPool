...
class _ThreadFetcher(Thread):
    ...
    def run(self):
        ...
        try:
            for proxy, proxy_type in self.fetcher():
                #self.log.info('ProxyFetch - %s: %s ok' % (self.fetch_source, proxy.ljust(23)))
                proxy = proxy.strip()
                if proxy in self.proxy_dict:
                    #print(self.fetch_source)
                    self.proxy_dict[proxy].add_source(self.fetch_source, proxy_type.upper())
                else:
                    self.proxy_dict[proxy] = Proxy(
                        proxy, source=self.fetch_source, proxy_type=proxy_type.upper())
