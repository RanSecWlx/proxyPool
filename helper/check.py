...
class _ThreadChecker(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.work_type == "raw" and self.proxy_handler.exists(proxy):
                continue
    ...
    def __ifRaw(self, proxy):
        if proxy.last_status:
            if self.proxy_handler.exists(proxy):
                self.log.info('RawProxyCheck - {}: {} {} {} exist'.format(self.name, proxy.proxy.ljust(23), proxy.proxy_type, proxy.source))
            else:
                self.log.info('RawProxyCheck - {}: {} {} {} pass'.format(self.name, proxy.proxy.ljust(23), proxy.proxy_type, proxy.source))
                proxy.create_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                self.proxy_handler.put(proxy)
        #else:
        #    self.log.info('RawProxyCheck - {}: {} {} {} fail'.format(self.name, proxy.proxy.ljust(23), proxy.proxy_type, proxy.source))

    def __ifUse(self, proxy):
        if proxy.last_status:
            self.log.info('UseProxyCheck - {}: {} {} {} pass'.format(self.name, proxy.proxy.ljust(23), proxy.proxy_type, proxy.source))
            #proxy.create_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            self.proxy_handler.put(proxy)
        else:
            fail_rate = proxy.fail_count / proxy.check_count
            #print(fail_rate, self.conf.maxFailRate, proxy.fail_count, proxy.check_count)
            if proxy.check_count >= 5 and fail_rate >= self.conf.maxFailRate:
            #if proxy.fail_count > self.conf.maxFailCount:
                self.log.info('UseProxyCheck - {}: {} {} {} fail, count {} {} delete'.format(self.name,
                                                                                    proxy.proxy.ljust(23), proxy.proxy_type, proxy.source, 
